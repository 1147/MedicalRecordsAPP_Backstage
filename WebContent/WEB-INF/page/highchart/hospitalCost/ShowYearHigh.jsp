  ...
  <script type="text/javascript">
	  $(function () {
		  	...
		  	$.ajax({
				type : "post",
				dataType : "json",
				url : '${ctx}/dep/ment/hospitalCostShow!getLiString?timeYear='+timeYear,
				success : function(data) {
					...
					$.each(data, function(idx, item) {
						//输出  
						$.each(item, function(idxx, itemm) {
						if(itemm.month!=2016 && itemm.month!=2015){
							date.push(itemm.number);
						}
						})
						if(item.month==2016){
							name = item.number;
					...
					var date2 = date.slice(0, 12);
					chart.addSeries({
						name : bname+'年',
						data : date2
					});
					var date2 = date.slice(12, 24);
					chart.addSeries({
						name : name+'年',
						data : date2
					});
					chart.setTitle({ text: bname+'年~'+name+'年 医疗费用统计' });
				},
				error : function(e) {
					alert(e);
				}
...
<div class="navbar navbar-default" id="navbar">
			...
			<input type="hidden" value="${timeYear }" id="timeYear">
				...
				<div class="main-content">
					...
					<div class="page-content">
						<div class="input-group">
							...
							<form action="hospitalCostShow!ShowYearHigh" id="guoaomen2">
								<div class="page-header">
								<%-- <button type="button" class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="bottom" title="未就诊与就诊信息统计">
									<s:url id="url" action="highchart" method="showRegHighchart">
									</s:url> <s:a href="%{url}" style="color:white;">就诊状态</s:a>
								</button>
								<button type="button" class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="bottom" title="医生热度统计">
									<s:url id="url" action="highchart" method="showDocHighchartFirst">
									</s:url> <s:a href="%{url}" style="color:white;">医生热度</s:a>
								</button>
								<button type="button" class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="bottom" title="科室热度统计">
									<s:url id="url" action="highchart" method="showDepartmentHighchart">
									</s:url> <s:a href="%{url}" style="color:white;">科室热度</s:a>
								</button> --%>
								请选择要统计的年份：<input type="text" name="timeYear" data-date-format="yyyy" id="datetimepicker" class="form-control date-picker" data-toggle="tooltip" data-placement="bottom" title="请选择查询的年份" style="width:150px;" />
...
<![endif]-->
		...
		<script src="${ctx}/assets/js/fuelux/fuelux.spinner.min.js"></script>
		<script src="${ctx}/assets/js/date-time/bootstrap-datepicker.min.js" charset="UTF-8"></script>
