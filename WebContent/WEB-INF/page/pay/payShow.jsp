							...
							<form class="form-search">
								<span class="input-icon">
									...
									<hr>
											...
											<form action="pay">
												...
												查询条件：<select  id="form-field-select-1" name="id" >
																				...
																				<option value="2">药品（设备）使用信息编号</option>
																				<option value="3">费用</option>
																				<option value="4">当前时间</option>
												 	</select>
											 	</span>
											 	<span class="col-xs-3">
											...
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												<thead>
													<tr>
														...
														<th style="text-align:center;">药品（设备）使用信息编号</th>	
												...
												<s:iterator value="pagination.list">
													<tr>
														<td id="dialog" style="text-align:center;">${pay_ID }</td>
														<td style="text-align:center;"><a href="javascript:;" onclick="druguseOnclick('${druguse_ID }');">${druguse_ID }</a></td>
														<td style="text-align:center;">${pay_cost }</td>
														<td style="text-align:center;">${pay_time }</td>
														<td style="text-align:center;">
															<c:if test="${pay_state==1 }"><span class="label label-success arrowed">已支付</span></c:if>
															<c:if test="${pay_state==0 }"><span class="label label-danger arrowed-in">未支付</span></c:if>
														</td>
													</tr>
												</s:iterator>
												</tbody>
									...
									<jsp:include page="pagination_util.jsp">
										<jsp:param value="pay?id=${id }&search_id=${search_id}" name="url"/>
									</jsp:include>
								</div>
...
<![endif]-->
		...
		<script type="text/javascript">
		function druguseOnclick(e){
			layer.open({
			    type: 2,
			    title: '药品使用信息',
			    skin: 'layui-layer-rim', //加上边框
			    area: ['500px', '400px'], //宽高
			    content: '${ctx}/dep/ment/dialogShow!showDrugUseByID?druguse.druguse_ID='+e
			});
		}
		</script>
