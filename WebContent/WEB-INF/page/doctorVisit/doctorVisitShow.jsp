						...
						<div class="nav-search" id="nav-search">
							<form class="form-search">
								<span class="input-icon">
									...
									<hr>
											<div >
											<form action="doctorVisit">
												<span class="col-xs-3">
												查询条件：<select  id="form-field-select-1" name="id" onchange="setDelType()">
																				<option value="0">--请选择--</option>
																				<option value="1">编号</option>
																				<option value="2">医生编号</option>
																				<option value="3">时间类型</option>
																				<option value="4">可挂号数量</option>
																				<option value="5">反馈状态</option>
																				<option value="6">时间状态</option>
												 	</select>
											 	</span>
											 	<div class="col-xs-3" id="guoaomen">
											 		<%-- <input type="text" id="guoaomen" class="form-control search-query" placeholder="请输入" name="search_id" value="${search_id }"/> --%>
											 		<input type="text" name="search_id" placeholder="请输入" id="id-date-picker-1" class="form-control search-query"  value="${search_id }"  />
												</div>
												<span class="col-xs-3">
										...
										<div class="table-responsive">
											...
											<table id="sample-table-1" class="table table-striped table-bordered table-hover">
												...
												<s:iterator value="pagination.list">
													<tr>
														<td id="dialog">${doctorVisit_ID }</td>
														<td ><a href="javascript:;" onclick="myOnclick('${doctorInfo_ID }');">${doctorInfo_ID }</a></td>
														<td>${doctorVisit_date }</td>
														<td>${doctorVisit_timeType }</td>
														<td>${doctorVisit_regNunber }</td>
														<td>${doctorVisit_feedback }</td>
														<td>
														 <c:if test="${doctorVisit_confirmState==1}">
															 <span class="label label-success arrowed">已确认</span></c:if>
														 <c:if test="${doctorVisit_confirmState==0}">
															 <span class="label label-warning">未确认</span></c:if>
														</td>
														<td>
															<c:if test="${doctorVisit_state==0}">
																 <span class="label label-success arrowed">未超时</span></c:if>
															<c:if test="${doctorVisit_state==1}">
																 <span class="label label-warning">已超时</span></c:if>
														</td>
														<td>
															<button class="btn btn-warning btn-xs">
															<i class="icon-wrench  bigger-110 icon-only"></i>
															<s:url id="url" action="doctorVisit" method="modifyDoctorVisitBefore">
																<s:param name="doctorVisit.doctorVisit_ID" value="doctorVisit_ID" />
